...
import { ChevronDown, AlertCircle, CheckCircle2, LayoutDashboard, Users, Shield, UserPlus } from 'lucide-react';
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-8">
        <div>
          ...
          <p className={`font-medium mt-1 ${d ? 'text-slate-400' : 'text-slate-500'}`}>Agrega, edita o elimina líderes.</p>
        </div>
        {isAdmin && (
          <div className="flex flex-wrap gap-3 w-full md:w-auto">
            <button onClick={() => setShowForm(!showForm)}
              ...
              <UserPlus className="w-4 h-4" /> {showForm ? 'Ocultar Formulario' : '+ Nuevo Líder'}
            </button>
          </div>
        )}
      </div>
      ...
      {isAdmin && showForm && (
        <div className={`rounded-[24px] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.05)] border p-6 sm:p-8 relative overflow-hidden mb-8 animate-in slide-in-from-top-4 fade-in duration-300 ${d ? 'bg-[#1e293b] border-slate-700' : 'bg-white border-slate-100'}`}>
