...
export const BusquedaView = ({
  ...
  return (
    <div className={`animate-in fade-in duration-500 w-full max-w-5xl mx-auto ${isBuscando ? 'space-y-6' : ''}`}>
      ...
      <div className={`transition-all duration-300 relative overflow-hidden group focus-within:shadow-md ${
        isBuscando 
        ...
        {!isBuscando && (
          <div className="w-full text-left mb-6">
              <h2 className={`text-2xl font-bold tracking-tight ${d ? 'text-white' : 'text-slate-900'}`}>Búsqueda Centralizada</h2>
          </div>
        )}
        ...
        <div className={`relative group w-full ${!isBuscando ? 'mb-12' : ''} z-10`}>
          ...
          <input type="text" placeholder="Buscar por nombre, documento de identidad, o ubicación..."
                ...
                className={`w-full pl-14 pr-4 rounded-[12px] transition-all outline-none font-medium ${
                  d 
        ...
        {!isBuscando && (
          <div className="flex flex-col items-center mt-4 mb-4 opacity-90 animate-in fade-in duration-300">
              ...
              <h3 className={`text-xl font-bold mb-2 ${d ? 'text-white' : 'text-slate-800'}`}>Comience su Investigación Electoral</h3>
